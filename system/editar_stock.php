...
// Report all errors except E_NOTICE
	...
	if(!empty($_POST)){
		$alert = '';
		if(empty($_POST['idproveedor']) || empty($_POST['idarticulo']) || empty($_POST['tipo_comprobante']) || empty($_POST['serie_comprobante']) || empty($_POST['num_comprobante']) || empty($_POST['existencia']) || empty($_POST['precio_compra']) || empty($_POST['precio_venta'])){
			...
			// 			WHERE (concat(compras.serie_comprobante,'-', compras.num_comprobante) = '$serie_num_comprobante' AND idcompras!= '$idcompras')";
			...
			// 		$sql_update_compras = "UPDATE compras 
			...
			// 		$sql_update_detalle_compras = "UPDATE detalle_compra
			...
			// 		$query_update2 = mysqli_query($con, $sql_update_detalle_compras);

			// 	if($query_update1 == 'true' && $query_update2 == 'true' ){
	...
	// Si no llega el Id
	if(empty($_GET['id'])){
		header('location: listado_stock.php');
	}

	$idcompras = $_GET['id'];

	$query_compras = "SELECT compras.idproveedor, proveedor.nombre AS nombre_proveedor, compras.idarticulo,articulo.nombre AS nombre_articulo, compras.tipo_comprobante, compras.serie_comprobante, compras.num_comprobante, compras.existencia, compras.precio_compra, compras.precio_venta, compras.idusuario, user.nombre AS nombre_usuario
		FROM compras AS compras
		JOIN proveedor as proveedor ON compras.idproveedor = proveedor.idproveedor
		...
		JOIN usuario as user ON compras.idusuario = user.idusuario
				WHERE compras.idcompras = '$idcompras' AND compras.estatus = 1";
		...
		<section id="container">
			...
			<div class="form_register">
				...
				<form action="" method="POST">
					...
					<div class="row">
						<div class="col-sm-4">
							<div class="input-group mb-3">
								...
								<input type="text" name="precio_venta" id="precio_venta" class="form-control text-center" placeholder="Precio Venta..." value="<?php echo $precio_venta; ?>" aria-label="Precio Venta" aria-describedby="inputGroup-sizing-default">
							</div>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-sm-6">
							<input type="submit" value="Actualizar Stock" class="btn btn-primary">
						</div>
					</div>
